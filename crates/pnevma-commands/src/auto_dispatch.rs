...
/// project config.
pub fn start_auto_dispatch(state: Arc<AppState>) {
    tokio::spawn(async move {
        // Wait a moment for the app to fully initialize before polling.
        tokio::time::sleep(Duration::from_secs(5)).await;

        loop {
            let interval = run_cycle(&state).await;
...
async fn run_cycle(state: &AppState) -> u64 {
    ...
    let tasks = match commands::list_tasks(state).await {
        Ok(t) => t,
        Err(e) => {
    ...
    let ready: Vec<_> = tasks
        .into_iter()
        .filter(|t| t.status == "Ready" && t.auto_dispatch)
        .collect();
