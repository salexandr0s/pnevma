...
- Reworking the release-hardening priority before the hardening exit bar is met.
...
## Recommended posture while hardening is active

Because `docs/hardening-exit-criteria.md` currently freezes feature work, this should be treated as a **post-hardening product plan**, not an immediate implementation commitment.

The practical message is:

- **During hardening:** keep shipping reliability, session recovery, dispatch, replay, and release quality work that strengthens the future command-center story.
- **After hardening exits:** start with Phase 1 above.
